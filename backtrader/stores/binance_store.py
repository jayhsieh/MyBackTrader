...
from ..feeds import BinanceData


class BinanceStore(object):
    """
    store 物件負責實作連線，部分功能與 broker 分不清楚，這部分待重構
    """
    _GRANULARITIES = {
    ...
    def __init__(self, api_key, api_secret, coin_refer, coin_target, testnet=False, retries=5):
        ...
        :param api_key: 從 Binance 平台上可以查詢
        :param api_secret: 從 Binance 平台上可以查詢
        :param coin_refer: 商品
        :param coin_target: 計價幣別
        :param testnet: 目前測試往段會失敗
        :param retries: 重試次數
        """
        ...
        # default is false, 要在 start 前設定
        # Daemon thread是一種在背景執行的thread，具有和main thread一同終止的特性
    ...
    # ================================================================
    # 只要有 retry 裝飾器的都是有與 binance service 交互的函數
    # ================================================================
    @retry
