...
from backtrader.stores import redisstore
from backtrader.utils.py3 import (queue, with_metaclass)


class MetaRedisData(DataBase.__class__):
    def __init__(cls, name, bases, dct):
        """Class has already been created ... register"""
        # Initialize the class
        super(MetaRedisData, cls).__init__(name, bases, dct)

        # Register with the store
        redisstore.RedisStore.DataCls = cls


class RedisData(with_metaclass(MetaRedisData, DataBase)):
    """Redis Data Feed.
    ...
    def __init__(self, **kwargs):
        ...
        """Status whether reconnecting in live state"""
        self._storedmsg = dict()
        """keep pending live message (under None)"""
        self.qlive = []
        """
        Heap queue of price from streamer of data store
        """
        self.o = self._store(**kwargs)
        """Data store"""

        # Subscribe data
        if self.p.data_freq is None:
            raise KeyError(f"Cannot read the 'data_freq' as {self.p.data_freq}")
        self.o.subscribe(self.p.dataname, self.p.data_freq)
    ...
    def setenvironment(self, env):
        ...
        belongs to
        """
        super(RedisData, self).setenvironment(env)
        env.addstore(self.o)

    def start(self):
        """
        Starts the Redis connection and gets the real contract and
        contract details if it exists
        """
        super(RedisData, self).start()
    ...
    def _st_start(self, instart=True, tmout=None):
        ...
        self.qlive = self.o.streaming_prices(self.p.dataname, self.p.data_freq)
        if instart:
    ...
    def _load(self):
        ...
        while True:
            if self._state == self._ST_LIVE:
                # Sleep one second if store did not run even once
                if not self.o.streaming_status():
                    continue

                try:
                    msg = heappop(self.qlive)[-1]
                except queue.Empty:
    ...
    def _load_tick(self, msg):
        """
        Loads tick or bar from 'msg' to 'lines'.
        :param msg: Tick or bar message from Redis
        :return:
        """
        ...
        self.lines.openinterest[0] = msg['open_interest']
